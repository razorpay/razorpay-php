...
class RegisterEmandateTest extends TestCase
{
    ...
    private $orderId = "order_JsZbs5MYbm60al";
    ...
    {
        $attributes = json_encode(array ('type' => 'invoice', 'date' => time(), 'customer_id'=> $this->customerId, 'line_items'=>array(array("name"=> "Master Cloud Computing in 30 Days", "amount"=>10000, "currency" => "INR", "quantity"=> 1))));

        Request::addHeader('Content-Type', 'application/json');

        $invoice = $this->api->invoice->create($attributes);
